...
def split_sentence_u(str):
    ...
    def marker_exception(loc, str):
        if loc == 0 or loc >= len(str)-2:
            return False
        if str[loc-1] == u'(' and str[loc+1] == u')':
            return True
        return False
    ...
    for i, char in enumerate(str):
        ...
        if char in [u'!', u'?', u';'] and not marker_exception(i, str):
...
def extract_related(text, keyword):
